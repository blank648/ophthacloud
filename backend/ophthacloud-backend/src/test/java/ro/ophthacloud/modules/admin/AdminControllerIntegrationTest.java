 ...
 * <p>
 * Keycloak is mocked via {@link MockitoBean} so no real Keycloak server is needed.
...
class AdminControllerIntegrationTest extends BaseIntegrationTest {
    ...
    void createStaff_shouldReturn201_withMockedKeycloak() {
        ...
        ResponseEntity<Map<String, Object>> response = client.post()
                ...
                .headers(h -> h.addAll(headersForRole("ADMIN", TENANT_A)))
        ...
        Map<String, Object> body = Map.of(
                ...
                "permissions", List.of(
                        Map.of("moduleCode", "patients", "canView", true, "canCreate", false,
                                "canEdit", false, "canDelete", false, "canSign", false, "canExport", false),
                        Map.of("moduleCode", "appointments", "canView", true, "canCreate", true,
                                "canEdit", true, "canDelete", false, "canSign", false, "canExport", false)
                )
        );
    ...
    void updateSettings_shouldReturn400_forInvalidSlotMinutes() {
        ...
        Map<?, ?> error = (Map<?, ?>) response.getBody().get("error");
        assertThat(error).isNotNull();
        // IllegalArgumentException from ClinicSettingsService → "BAD_REQUEST" per GlobalExceptionHandler
